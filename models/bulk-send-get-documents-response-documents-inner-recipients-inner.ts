/*
Resources and endpoints

When I started SignWell in 2019, I saw there was a need for an alternative to the hard-to-use and expensive e-signature software already out there. Documents can be complicated enough, but getting a document signed shouldn't be complicated too.

At SignWell, we pride ourselves not only on the ease and affordability of our e-signature process but also on our personalized and industry-leading customer support — whether it's for individual use or larger team accounts, SignWell is here to help you feel comfortable and confident getting your documents signed.

The SignWell mission? Simplify how documents get signed for millions of people and businesses. We're excited to help you continue to move toward the future of paperless document signing.

Ruben Gamez
Founder, SignWell

The version of the OpenAPI document: 1


NOTE: This file is auto generated by Konfig (https://konfigthis.com).
*/
import type * as buffer from "buffer"

import { BulkSendGetDocumentsResponseDocumentsInnerRecipientsInnerAttachmentRequestsInner } from './bulk-send-get-documents-response-documents-inner-recipients-inner-attachment-requests-inner';

/**
 * 
 * @export
 * @interface BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
 */
export interface BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner {
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'email'?: string;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'id'?: string;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'message'?: string | null;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'name'?: string;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'passcode'?: string | null;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'send_email'?: string | null;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'send_email_delay'?: string | null;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'status'?: string;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'subject'?: string | null;
    /**
     * 
     * @type {number}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'signing_order'?: number;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'signing_url'?: string;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'bounced'?: string | null;
    /**
     * 
     * @type {string}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'bounced_details'?: string | null;
    /**
     * 
     * @type {Array<BulkSendGetDocumentsResponseDocumentsInnerRecipientsInnerAttachmentRequestsInner>}
     * @memberof BulkSendGetDocumentsResponseDocumentsInnerRecipientsInner
     */
    'attachment_requests'?: Array<BulkSendGetDocumentsResponseDocumentsInnerRecipientsInnerAttachmentRequestsInner>;
}

