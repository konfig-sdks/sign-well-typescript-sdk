...
import { readableStreamToString, SignWellError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SignWellError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SignWellError(e, e.response?.data)
                }
                if (innerError instanceof SignWellError) {
                    // Got "SignWellError" from the above try block
                    throw innerError;
                }
